...
const defaultCols = Object.keys(TASKS_STATUS)
    .map(k => ({ id: k, title: TASKS_STATUS[k as TaskStatus] })) satisfies Column[];

export function KanbanBoard({ tasks: tasksReceived }: { tasks: ITask[] }) {
    ...
    const announcements: Announcements = {
        onDragStart({ active }) {
            ...
            if (active.data.current?.type === "Column") {
                ...
                return `Picked up Task ${active.data.current.task.status
    ...
    function onDragEnd(event: DragEndEvent) {
        ...
        if (isActiveAColumn) {
            ...
            if (activeData && pickedUpTaskColumn.current) {
                const newColumn = activeData.task.status as TaskStatus
                const oldColumn = pickedUpTaskColumn.current as TaskStatus
                const oldPosition = activeData?.task.order as number
                ...
                if (newColumn !== oldColumn || taskPosition !== oldPosition) {
                    const tasksToUpdate: Pick<ITask, 'id' | 'status' | 'order'>[] = [
                    ...
                    if (newColumn !== oldColumn) {
                        const tasksInOldColumn = tasks.filter((task) => task.status === oldColumn);
                        tasksToUpdate.push(...tasksInOldColumn.map((t, index) => ({
                            id: t.id,
                            status: t.status,
                            order: index
                        })))
                    ...
                    Promise.allSettled(tasksToUpdate.map(t => updateTask(t.id, { status: t.status, order: t.order })))
                }
            }
        }
    }
